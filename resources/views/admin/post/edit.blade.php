...
@section('content')
    ...
    <form action="{{route('post.update',$post->id)}}" method="post" enctype="multipart/form-data">
        <div class="col-md-9">
            ...
            <img src="{{url('/')}}/uploads/post/{{$post->image}}" class="img-thumbnail" width="250px"><br><br>
            <input type="file" name="image" class="btn btn-primary">
            <br>
            <br>
            <span class="label">Başlıq</span>
            <br>
            <input type="text" name="title" class="form-control" placeholder="Başlıq" value="{{$post->title}}">
            <br>
            <textarea name="post_content" id="post-content" >{{$post->content}}</textarea>
            ...
            <input type="text" name="description" class="form-control" placeholder="Açıqlama" value="{{$post->description}}">
            <br>
            ...
            <select name="gallery"  class="form-control">
                ...
                @foreach($gallery as $gallery)
                    <option value="{{$gallery->id}}" {{$gallery->id==$post->gallery_id ? 'selected' : ''}}>{{$gallery->name}}</option>
                @endforeach
            </select>

            <br>
            <br>
            <input type="submit" value="Dəyişdir" class="btn btn-primary btn-lg">
        </div>
    </form>
